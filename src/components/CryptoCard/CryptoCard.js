    ...
    <div className="relative w-full my-3">
      {comingSoon && (
        <div className="bg-green-2 absolute z-20 top-1 left-0.5 p-2 w-1/3 rounded-full drop-shadow-sm">
          <p className="text-sm text-white leading-3 font-semibold h-full text-center">
            Coming soon
          </p>
        </div>
      )}
      <div
        className={`flex shadow-md ${
          ((evm && !enough_balance) || comingSoon) && "opacity-60"
        } rounded-md p-5 flex-row justify-between ring-2 w-full ${
          payment_method === id
            ? "ring-gray-400 bg-gray-100"
      ...
      >
        ...
        <div className="flex w-1/4 flex-col items-end">
          <p className="text-gray-400 text-sm">
            {(fiat_amount / price).toFixed(evm ? 2 : 10)}
          </p>
